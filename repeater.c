 ...
 * From version 0.08 onwards, function fatal() also sets stopped == TRUE to
 * achieve clean shutdown
 */
static bool stopped;
...
/* Parse IdCode string of format "ID:xxxxx", where xxxxx is some positive
 * (non-zero) long integer number
...
/* check intended Mode 1 server address against list of denied addresses/ranges
 * in repeater.ini
...
/* check intended Mode 1 server address against list of allowed
 * addresses/ranges in repeater.ini
...
/* Try to connect to vnc server, return connected socket if success, -1 if
 * error parameter serverIp holds server ip address on return
 * (or "" in case of error)
 */
...
/* Accept connection in non-blocking way (this should stop hanging if something
 * bad has happened to socket between select() and accept())
	 ...
	 */
		 ...
		 */
            ...
            if ((allowedModes & CONN_MODE1) > 0) {
                ...
                /* id is an [hostname / ip address] / [port number / display number]
		 * combination of some sort, parse it
		 */
        ...
        if (connMode == CONN_MODE1) {
			 ...
			 */
                        ...
                        /* we have run out of slots in server or viewer table,
